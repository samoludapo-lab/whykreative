...
function createStoryboard(input = {}, workflow) {
  ...
  return {
    ...
    blender: {
      sceneTemplate: enumValue(input.sceneTemplate, ["studio_turntable_reveal", "floating_gallery", "kinetic_feature_breakdown", "environmental_lifestyle"], "studio_turntable_reveal"),
...
function createScene(index, sceneCount, secondsPerScene, input, workflow, context) {
  ...
  return {
    ...
    assets: ["asset_hero"],
    camera: index === 0 ? cleanText(input.openingCamera, 220, "fast push-in from wide to hero angle") : cleanText(input.cameraMove, 220, "slow orbit with controlled parallax"),
    lighting: cleanText(input.lightingSetup, 260, "softbox key, rim light, controlled fill, contact shadows"),
    animation: finalScene ? cleanText(input.finalAnimation, 260, "hero lockup, logo-safe pause, subtle glow pulse") : cleanText(input.subjectMotion, 260, "turntable reveal with light sweep"),
    voice: {
      character: workflow.characterVoice,
